          ...
          {menuItem &&
            menuItem.map((item) => {
              const isActive =
                pathName == item?.link ||
                id == item.id ||
                item?.dropdownMenu?.some((item) => item.id == id) ||
                item?.dropdownMenu?.some((item) =>
                  item?.subMenu?.some((subItem) => subItem.id == id)
                );
              return (
                <li
                  key={item.id}
                  className={`flex relative  group p-2 m-0 hover:bg-orange-400  ${
                    isActive === true
                      ? "bg-[#FFB300] border-b-2  border-solid border-black"
                      : ""
                  }`}
                >
                  <Link
                    href={item?.id == 1 ? `/` : `/post/${item?.id}`}
                    className={`flex items-center justify-center uppercase font-bold text-balance  ${
                      isActive === true ? "text-red-500" : "text-white"
                    } hover:text-black  rounded-sm dark:bg-grtext-gray-600 md:dark:bg-transparent `}
                    aria-current="page"
                  >
                    {item?.title}
                    {item.dropdownMenu && (
